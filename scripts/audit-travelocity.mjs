/**
 * Live DOM audit for www.travelocity.com urgency / social-proof widgets.
 * Headed by default. No sign-in or checkout.
 * Run: node scripts/audit-travelocity.mjs
 * Writes temp/travelocity-audit.json
 */
...
const outPath = resolve(root, 'temp/travelocity-audit.json');
const HOST = 'https://www.travelocity.com';

const SERP =
  `${HOST}/Hotel-Search?destination=New%20York&startDate=2026-10-16&endDate=2026-10-17&adults=2&rooms=1`;
...
function dumpPage() {
  ...
  const NAG =
    /people are (looking|viewing)|viewing this|looking at this|booked|just booked|in the last|only \d+|we have \d+ left|only one left|left at this price|in high demand|in demand|hurry|ends in|expires|limited time|someone just|times on our site|rooms? left|on our site|still available|watching/i;
  ...
  const price = [...document.querySelectorAll('[data-stid*="price"], [data-testid*="price"], [class*="price"]')]
    .filter(vis)
    .slice(0, 3)
...
async function audit(name, url) {
  ...
  await page
    .waitForSelector('a[href*="Hotel-Information"], a[href*=".h"], h1, [data-stid="property-listing-results"]', {
      timeout: 25000,
    })
    .catch(() => {});
...
try {
  ...
  if (serp.blocked) {
    ...
    const href2 =
      href ||
      (await page
        .locator('a[href*="Hotel-Information"]')
        .filter({ hasNot: page.locator('[href*="hotelplanner"]') })
        .first()
        .getAttribute('href')
        .catch(() => null));
    const propertyHref = href2;
    if (!propertyHref) {
      report.pages.push({ name: 'pdp', error: 'no property link on SERP' });
    } else {
      const property = propertyHref.startsWith('http') ? propertyHref : `${HOST}${propertyHref}`;
